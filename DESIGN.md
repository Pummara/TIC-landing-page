...
**Two-font sections.** Services and Contact deliberately use only Display + Body (no mono) — Services' column headers and (formerly) its subtitle label, and Contact's Phone/Email/Address labels, were originally set in JetBrains Mono and read as thin/hard to read next to the bold display titles. These were moved to Body (Inter/Prompt) at `font-semibold` by direct instruction. Don't reintroduce mono there; it's a deliberate two-typeface zone, not an oversight.
...
- **Title** (700, 24px, tracking -0.01em, uppercase): service-row titles (Big Shoulders Display) and Contact's phone/email/address values (`font-[Prompt,sans-serif]` specifically, not Big Shoulders Display — see the Contact Band component doc for why).
...
- **Outline-gold tone:** transparent fill, solid vivid-gold border, white icon — **Trust Bar's 4 icons only** (Trusted/Professional/Fast & Efficient/Best Value), by direct instruction. Vivid gold is correct here per the Three-Gold Rule (a dark ground, `bg-ink`). **Why Choose Us keeps its 4 different icons on Outline-light** — don't let the two drift onto each other's tone; they're deliberately different now.
...
- Each row: icon chip, title (Big Shoulders Display) + description (body, slate). No code/tag column — removed by direct instruction; don't reintroduce a third column here.
- **No subtitle line.** `ServiceItem` originally carried a `subtitle` field (a short English tag under the title, e.g. "Car Rental & Transfer") that a `/impeccable critique` pass first patched to only render when it differed from the title — but that produced visible inconsistency across the four rows (two showed a gold subtitle line, two didn't, depending on whether that row's title happened to match its subtitle). Removed by direct instruction for consistency: `subtitle` no longer exists on `ServiceItem` (`src/i18n/types.ts`) or in any of the four items in `th.ts`/`en.ts` — every row is title + description only, uniformly. Don't reintroduce a subtitle field or its conditional-render guard; if a distinguishing tag is ever wanted again, it needs a fresh design decision, not a resurrection of the old field.
...
- **The value line is pinned to `font-[Prompt,sans-serif]`, not `font-display`.** Root cause this fixed: `font-display` (Big Shoulders Display, with Prompt only as its Thai-glyph fallback) meant pure-Latin phone/email values rendered fully in Big Shoulders Display while the Thai address value fell back entirely to Prompt — two unrelated typefaces side by side in the same row, which the site owner correctly flagged as "multiple fonts, weight not even." Forcing Prompt uniformly, regardless of script, was the fix — not `font-sans` (Inter/Prompt), which would just swap Inter in for Big Shoulders Display and reproduce the identical mismatch.
- **Phone has two numbers, independently clickable, email/address unchanged.** `contact.phone` carries a second `mobile` field (`081-564-5966`, added alongside the landline `02-433-0191`, by direct instruction, Contact-section-only — Header/Footer still show only the landline). Because one wrapping `<a>` can't hold two different `tel:` targets, the phone card alone renders as a plain `<div>` containing two independent `<a href="tel:...">` lines, instead of the whole-cell-is-one-link pattern email/address still use.
  - This means the phone card's keyboard-focus punch-through can't be driven by the wrapper's own `:focus-visible` (a `<div>` is never focusable) — its inner elements use `group-focus-within:*` instead of `group-focus-visible:*` (fires when *either* inner link is focused), and the card carries both `sweep` and a second, additive-only `sweep-focus-within` class (`src/index.css`) that extends the shared `.sweep` ink-fill trigger to `:focus-within`. **Never add `:focus-within` to the shared `.sweep` rule itself** — that would give every other `.sweep` element (buttons, Language Toggle, Header's mobile phone icon, Contact's own email card) mouse-click-inclusive focus behavior, regressing the documented `:focus-visible`-only nuance system-wide. `.sweep-focus-within` is the pattern for any future card that nests more than one focusable child.
...
- **Every logo carries `loading="lazy"`** (added by an `/impeccable audit` pass) — this section sits near the bottom of the page, and with the marquee's duplicated track that's 26 images; deferring them until the section nears the viewport meaningfully cuts initial page weight (this section's images alone are a large share of the site's ~1.3MB image payload). Confirmed via network-request inspection that they genuinely don't fetch until scrolled near.
...
- Every logo carries `loading="lazy"` (added by an `/impeccable audit` pass, same rationale as Client Logos) — this section's 17 tiles are the other large share of the site's image weight and sit well below the fold.
...
- **Inactive:** dimmed ink or white (context-dependent) with a hover state. On the light tone (the only one actually in use, in `Header.tsx`), this is `text-ink/70` on white — an `/impeccable audit` pass found the original `/60` measured 4.11:1, failing WCAG AA (4.5:1) for the 14px label; `/70` clears 5.57:1 with real margin. The dark tone's `text-paper-white/60` already passed (6.1–6.9:1 against either ink ground) and was left unchanged.
- Each tab is `py-3` (was `py-1.5`) so the tap target is 44px tall, matching the audit's touch-target recommendation — don't shrink this back down for visual tightness.
...
- **The Punch-Through Sweep** (`.sweep` in `src/index.css`) — hover/focus feedback on every solid or bordered clickable element (Buttons, Contact's three cells, Header's mobile icon-square phone link, Language Toggle's inactive tab). Added by direct instruction to animate hover/click feedback across the system. Rather than a flat color crossfade, the fill wipes in from the leading edge via a `::before` pseudo-element (`transform: scaleX(0)→1`, `200ms cubic-bezier(0.16, 1, 0.3, 1)`, origin left) — a departures-board flap flipping to its next state, not a generic hover fade. `--sweep-bg` is set per usage via a Tailwind arbitrary property referencing the token file (e.g. `[--sweep-bg:theme(colors.ink.DEFAULT)]`, `[--sweep-bg:theme(colors.gold.DEFAULT/15%)]`) rather than a raw hex/rgba literal — an `/impeccable audit` pass converted the one remaining raw-rgba instance (Language Toggle's preview fill) to this pattern so a future gold re-pick (already happened twice this project) propagates automatically. The pseudo-element sits at `z-index: -1` inside a `.sweep`-isolated stacking context, which paints it above the element's own background but below normal in-flow content — no extra wrapper needed for labels/icons to stay legible. Triggers on both `:hover` and `:focus-visible`, so keyboard users get identical feedback to pointer users. Header's mobile icon-square phone link is `h-11 w-11` (44px, up from `h-9 w-9`/36px) — the same audit pass's touch-target fix; don't shrink it back for visual tightness, it's the only way to call on mobile.
...
- **Do** use `group-focus-within:*` plus the `.sweep-focus-within` opt-in class (not the shared `.sweep` rule) for any card that nests more than one independently-focusable child, the way Contact's phone card does for its two `tel:` links.
- **Do** keep Contact's second phone number (`contact.phone.mobile`) scoped to the Contact section only — Header and Footer intentionally still show just the landline, by direct instruction.
...
- **Don't** reintroduce JetBrains Mono on Services' column-headers or Contact's labels — moved to Body font by direct instruction (see Typography's Two-font sections note).
...
- **Don't** let Trust Bar and Why Choose Us share an icon-chip tone — Trust Bar is `outline-gold`, Why Choose Us stays `outline-light`, by direct instruction.
- **Don't** reintroduce `font-display` on Contact's phone/email/address value line, or swap it for `font-sans` — both reproduce the original two-typeface mismatch. It stays `font-[Prompt,sans-serif]`.
- **Don't** add `:focus-within` to the shared `.sweep` rule in `src/index.css` to solve a future multi-child sweep card — use a scoped opt-in class instead, the way `.sweep-focus-within` does for Contact's phone card. Editing the shared rule would silently change every other `.sweep` element's focus behavior sitewide.
- **Don't** translate the footer copyright into Thai — English in both locales by direct instruction, same pattern as Contact's labels and the footer's column headings.
- **Don't** reintroduce a `subtitle` field on Services' `ServiceItem` — removed by direct instruction because conditionally showing it on only 2 of 4 rows (depending on whether that row's title happened to match its subtitle) read as inconsistent. Every row is title + description only now.
- **Don't** shrink Language Toggle's tabs back to `py-1.5` or its inactive-tab text back to `/60` opacity — an `/impeccable audit` pass found both failed real, verified checks (a 32px touch target, a 4.11:1 contrast ratio against the 4.5:1 AA minimum).
- **Don't** remove `loading="lazy"` from Client Logos' or Partner Airlines' images, and don't add it to the Hero photo or the Header/Footer logos — the lazy set is specifically the below-the-fold bulk of the site's ~1.3MB image payload; the eager set is above-the-fold or tiny, and the Hero photo in particular is the LCP candidate.
- **Don't** write a new raw hex/rgba color literal in a Tailwind arbitrary value or in `index.css` — reference the token file via `theme('colors.x.y')` (plain CSS) or `theme(colors.x.y)` (Tailwind arbitrary-value brackets) instead, even at a fractional opacity (`theme('colors.ink.DEFAULT / 8%')`). An `/impeccable audit` pass converted every remaining instance; a new literal reintroduces exactly the drift that pass removed.
